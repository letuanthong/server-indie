...
public class AlyraManager {
    ...
    static {
        ...
        if (DRIVER == null || DB_HOST == null || DB_PORT == null || DB_USER == null || DB_NAME == null
                || DB_NAME_DATA == null) {
    ...
    private static void loadProperties() {
        ...
        try {
            ...
            if ((value = properties.get("database.min")) != null) {
                AlyraManager.MIN_CONN = Integer.parseInt(String.valueOf(value).trim());
            }
            if ((value = properties.get("database.max")) != null) {
                AlyraManager.MAX_CONN = Integer.parseInt(String.valueOf(value).trim());
            }
            if ((value = properties.get("database.lifetime")) != null) {
                AlyraManager.MAX_LIFE_TIME = Integer.parseInt(String.valueOf(value).trim());
            }
    ...
    private static HikariConfig createConfig(String poolName, String databaseName) {
        String jdbcUrl = String.format(
                "jdbc:mariadb://%s:%s/%s?useUnicode=yes&characterEncoding=UTF-8&allowPublicKeyRetrieval=true",
