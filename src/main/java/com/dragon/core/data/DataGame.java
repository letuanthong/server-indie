...
public class DataGame {
    ...
    public static void updateData(MySession session) {
        ...
        if (dart == null || arrow == null || effect == null || image == null || part == null || skill == null) {
            Logger.warning("One or more update data files not found");
            return;
        }
    ...
    // thonk Ham comment trong try nhung khong biet ly do gi
    public static void sendDataImageVersion(MySession session) {
        Message msg;
        try {
           msg = new Message(-111);
           msg.writer().writeShort(0);
           msg.writer().writeUTF("DragonServer");
           msg.writer().writeByte(0);
           msg.writer().writeUTF("NgocRongDev");
           msg.writer().writeByte(1);
           msg.writer().writeUTF("Dev1sme_ImageDataVersion");
    ...
    public static void sendIcon(MySession session, int id) {
        ...
        try {
            ...
            if (icon == null) {
                Logger.warning("Icon file not found: data/icon/x" + session.zoomLevel + "/" + id + ".png");
                return;
            }
            msg = new Message(-67);
...
//            }
            ...
            if (mob == null) {
                Logger.warning("Mob file not found: data/mob/x" + session.zoomLevel + "/" + id);
                return;
            }
            msg = new Message(11);
            msg.writer().writeByte(id);
    ...
    public static void sendTileSetInfo(MySession session) {
        ...
        try {
            ...
            if (data == null) {
                Logger.warning("Tile set info file not found: data/map/tile_set_info");
                return;
            }
            msg = new Message(-82);
    ...
    public static void sendImageByName(MySession session, String imgName) {
        ...
        try {
            ...
            if (data == null) {
                Logger.warning("Image file not found: data/img_by_name/x" + session.zoomLevel + "/" + imgName + ".png");
                return;
            }
    ...
    public static void sendRes(MySession session) {
        ...
        try {
            ...
            if (files == null) {
                Logger.warning("Resource directory not found: data/res/x" + session.zoomLevel);
                return;
            }
            for (final File fileEntry : files) {
                ...
                if (res == null) {
                    Logger.warning("Resource file not found: " + fileEntry.getAbsolutePath());
                    continue;
                }
                msg = new Message(-74);
                msg.writer().writeByte(2);
    ...
    public static void sendLinkIP(MySession session) {
        ...
        try {
            ...
            msg.writer().writeByte(1);
            Logger.log(Logger.CYAN, "[LINK IP] IP: " + session.ipAddress + " - Sending LinkIP: " + LINK_IP_PORT + ", ver=" + session.version + "\n");
            session.sendMessage(msg);
            msg.cleanup();
        } catch (IOException e) {
            Logger.logException(DataGame.class, e);
