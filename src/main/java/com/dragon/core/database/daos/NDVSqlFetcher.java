...
import com.dragon.model.player.Fusion;
...
import com.dragon.system.AntiLogin;
...
import com.dragon.system.Template.AchievementQuest;
...
public class NDVSqlFetcher {
    ...
    public static Player login(MySession session, AntiLogin al) {
        ...
        Player plInGame;
        try {
            Logger.log(Logger.CYAN, "[DB] Querying account for: " + session.uu + "\n");
            rs = AlyraManager.executeQuery("select * from account where username = ? and password = ?", session.uu, session.pp);
            if (rs.first()) {
                Logger.log(Logger.CYAN, "[DB] Account found for: " + session.uu + "\n");
                session.userId = rs.getInt("account.id");
                session.isAdmin = rs.getBoolean("is_admin");
//                 if (!session.isAdmin) {
//                            Client.gI().kickSession(session);
//                            Service.gI().sendThongBaoOK(session, "Hiện chỉ có admin mới có thể đăng nhập");
//                        }
                ...
                if (rs.getBoolean("ban")) {
                    ...
                    if (secondsPass < Manager.SECOND_WAIT_LOGIN) {
                        Logger.log(Logger.YELLOW, "[DB] " + session.uu + " - Wait to login: " + (Manager.SECOND_WAIT_LOGIN - secondsPass) + "s\n");
                        Service.gI().sendWaitToLogin(session, Manager.SECOND_WAIT_LOGIN - secondsPass);
                    } else {
                        Logger.log(Logger.CYAN, "[DB] " + session.uu + " - Querying player data...\n");
                        rs = AlyraManager.executeQuery("select * from player where account_id = ? limit 1", session.userId);
                        if (!rs.first()) {
                            Logger.log(Logger.YELLOW, "[DB] " + session.uu + " - No character, switching to create char\n");
                            ...
                            if (plInGame != null) {
                                Logger.log(Logger.YELLOW, "[DB] " + session.uu + " - Kicking existing session\n");
                                Client.gI().kickSession(plInGame.getSession());
                            }
                            Logger.log(Logger.CYAN, "[DB] " + session.uu + " - Loading player data...\n");
                            if ((player = loadPlayer(rs, false)) != null) {
                                Logger.log(Logger.GREEN, "[DB] " + player.name + " - Player loaded successfully\n");
                                ...
                                Logger.log(Logger.RED, "[DB] " + session.uu + " - loadPlayer returned null!\n");
                            }
                        }
                    }
                }
...
//    }
    ...
    private static Player loadPlayer(AlyraResultSet rs, boolean isOffline) throws Exception {
        ...
        try {
            ...
            player.inventory.coupon = dataArray.size() >= 4 ? Integer.parseInt(String.valueOf(dataArray.get(3))) : 0;
            player.inventory.event = dataArray.size() >= 5 ? Integer.parseInt(String.valueOf(dataArray.get(4))) : 0;
            dataArray.clear();
...
//            }
            ...
            try {
                ...
                if (dataArray != null && dataArray.size() >= 10) {
                    ...
                    System.out.println("dataArray is null or does not have enough elements.");
                }
            } catch (Exception e) {
                Logger.logException(NDVSqlFetcher.class, e);
            }
