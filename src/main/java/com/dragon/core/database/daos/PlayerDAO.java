...
public class PlayerDAO {
    //thonk
    //private final ScheduledExecutorService scheduler = Executors.newScheduledThreadPool(1); // Tạo một thread duy nhất cho scheduler
    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            AlyraManager.executeUpdate("insert into player"
                    ...
                    + "data_black_ball, data_side_task, BoughtSkill) "
                    ...
                    itemTime, task, mabuEgg, charms, skills, skillsShortcut, petData, dataBlackBall, dataSideTask, dataBoughtSkill);
        ...
        if (player != null && player.idMark.isLoadedAllDataPlayer()) {
            ...
            try {
                ...
                String query = "update player set head = ?, have_tennis_space_ship = ?, "
                        ...
                        + "rank = ?, data_achievement = ?, giftcode = ?, event_point = ?, data_event = ?, dataBadges = ?, dataTaskBadges = ?, BoughtSkill = ?, LearnSkill = ? where id = ?";
                ...
                if (player.isOffline) {
                    Logger.log(Logger.PURPLE, "[PLAYER] "+TimeUtil.getCurrHour() + "h" + TimeUtil.getCurrMin() + "m: Player " + player.name + " updated successfully! " + (System.currentTimeMillis() - st) + "ms\n");
                    player.dispose();
                } else {
                    Logger.success("[PLAYER] "+TimeUtil.getCurrHour() + "h" + TimeUtil.getCurrMin() + "m: Player " + player.name + " saved successfully! " + (System.currentTimeMillis() - st) + "ms\n");
                }
            } catch (Exception e) {
    ...
    public static boolean checkLogout(Connection con, Player player) {
        String query = "select last_time_logout, last_time_login from account where id = ? limit 1";
        try (PreparedStatement ps = con.prepareStatement(query)) {
            ...
            Logger.logException(PlayerDAO.class, e, "Lỗi check logout " + player.name);
        }
        return false;
    }
...
//        }
    ...
    public static boolean createPlAo(int userId, String name, byte gender, int hair) {
        try {
            ...
            dataArray.add(0); //giáp gốc
            dataArray.add(0); //chí mạng gốc
            dataArray.add(0); // Chí mạng rồng thần
            dataArray.add(0); //năng động
            ...
            AlyraManager.executeUpdate(
                    ...
                    + "items_bag, items_box, items_box_lucky_round, items_daban, friends, enemies, data_intrinsic, data_item_time, "
                    + "data_task, data_mabu_egg, data_charm, skills, skills_shortcut, pet, "
                    + "data_black_ball, data_side_task, masterDoesNotAttack) " // Thêm trường masterDoesNotAttack
                    + "values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", // Thêm giá trị cho masterDoesNotAttack
                    userId, name, hair, gender, 0, -1, inventory, location, point, magicTree,
                    itemsBody, itemsBag, itemsBox, itemsBoxLuckyRound, itemsDaBan, friends, enemies, intrinsic,
                    itemTime, task, mabuEgg, charms, skills, skillsShortcut, petData, dataBlackBall, dataSideTask, 0 // Giả sử giá trị mặc định là 0
            );
