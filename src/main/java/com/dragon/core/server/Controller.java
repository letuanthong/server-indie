...
import com.dragon.consts.ConstIgnoreName;
import com.dragon.consts.ConstMap;
import com.dragon.consts.ConstNpc;
import com.dragon.consts.ConstTask;
import com.dragon.content.daihoi.SuperRankService;
import com.dragon.core.data.AlyraManager;
import com.dragon.core.data.AlyraResultSet;
import com.dragon.core.data.DataGame;
import com.dragon.core.data.ItemData;
import com.dragon.core.database.daos.PlayerDAO;
import com.dragon.core.database.daos.SuperRankDAO;
import com.dragon.core.interfaces.IMessageHandler;
...
import com.dragon.services.ChatGlobalService;
import com.dragon.services.ConsignShopService;
import com.dragon.services.FlagBagService;
import com.dragon.services.ItemTimeService;
...
import com.dragon.services.dungeon.BlackBallWarService;
import com.dragon.services.dungeon.TrainingService;
import com.dragon.services.func.Input;
import com.dragon.services.func.LuckyRound;
import com.dragon.services.func.TransactionService;
import com.dragon.services.func.UseItem;
...
import com.dragon.services.map.NpcManager;
...
public class Controller implements IMessageHandler {
    ...
    public void onMessage(ISession s, Message _msg) {
        ...
        try {
            ...
            switch (cmd) {
                ...
                case -87 -> DataGame.updateData(_session);
                case -67 -> {
                    int id = _msg.reader().readInt();
                    DataGame.sendIcon(_session, id);
                }
                case 66 -> DataGame.sendImageByName(_session, _msg.reader().readUTF());
                case -66 -> {
                    if (player != null) {
                ...
                case -41 -> Service.gI().sendCaption(_session, _msg.reader().readByte());
                case -43 -> {
                    if (player != null) {
                        if (TransactionService.gI().check(player)) {
                            Service.gI().sendThongBao(player, "Không thể thực hiện");
                ...
                case -111 -> DataGame.sendDataImageVersion(_session);
                case -20 -> {
                ...
                case -28 -> messageNotMap(_session, _msg);
                case -29 -> messageNotLogin(_session, _msg);
                case -30 -> messageSubCommand(_session, _msg);
                case -15 -> {
                    // về nhà
                    if (player != null) {
                ...
                case -76 -> AchievementService.gI().confirmAchievement(player, _msg.reader().readByte());
                default -> {
                }
            }
    ...
    public void messageNotLogin(MySession session, Message msg) {
        if (msg != null) {
            try {
                ...
                switch (cmd) {
                    case 0 -> session.login(msg.reader().readUTF(), msg.reader().readUTF());
                    case 2 -> Service.gI().setClientType(session, msg);
                    default -> {
                    }
                }
            } catch (IOException e) {
    ...
    public void messageNotMap(MySession _session, Message _msg) {
        if (_msg != null) {
            ...
            try {
                ...
                switch (cmd) {
                    case 2 -> createChar(_session, _msg);
                    case 6 -> DataGame.updateMap(_session);
                    case 7 -> DataGame.updateSkill(_session);
                    case 8 -> ItemData.updateItem(_session);
                    case 10 -> DataGame.sendMapTemp(_session, _msg.reader().readUnsignedByte());
                    case 13 -> {
                        // client ok
    ...
    private void sendThongBaoServer(Player player) {
        Service.gI().sendThongBaoFromAdmin(player, "Welcome to Server" + ServerManager.NAME_SERVER + "\n");
    }
}
