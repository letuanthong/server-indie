...
public class ItemMap {
    ...
    public static ItemMap create(Zone zone, int tempId, int quantity, int x, int y, long playerId) {
        ItemMap item = new ItemMap(zone, tempId, quantity, x, y, playerId);
        zone.addItem(item);
        return item;
    }

    private ItemMap(Zone zone, int tempId, int quantity, int x, int y, long playerId) {
        this.zone = zone;
    ...
    public static ItemMap create(Zone zone, ItemTemplate temp, int quantity, int x, int y, long playerId) {
        ItemMap item = new ItemMap(zone, temp, quantity, x, y, playerId);
        zone.addItem(item);
        return item;
    }

    private ItemMap(Zone zone, ItemTemplate temp, int quantity, int x, int y, long playerId) {
        this.zone = zone;
        this.itemMapId = zone.countItemAppeaerd++;
        if (zone.countItemAppeaerd >= 2000000000) {
            zone.countItemAppeaerd = 0;
    ...
    public static ItemMap createCopy(ItemMap itemMap) {
        ItemMap item = new ItemMap(itemMap);
        itemMap.zone.addItem(item);
        return item;
    }

    private ItemMap(ItemMap itemMap) {
