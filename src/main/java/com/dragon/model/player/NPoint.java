...
import com.dragon.consts.ConstRatio;
...
import com.dragon.content.radar.Card;
...
public class NPoint {
    ...
    private void setHpMax() {
        ...
        long newHpMax = this.hpg + this.hpAdd;

        // Áp dụng các yếu tố ảnh hưởng đến hpMax
        for (Integer tl : this.tlHp) {
            newHpMax += (newHpMax * tl / 100L);
        }

        // Xử lý set nappa
        if (this.player.setClothes.nappa == 5) {
            newHpMax += (newHpMax * 80L / 100L);
        }

        if (this.player.setClothes.cadicM >= 2) {
            newHpMax += (newHpMax * 20L / 100L);

        }
        // Xử lý set worldcup
        if (this.player.setClothes.worldcup == 2) {
            newHpMax += (newHpMax * 10 / 100L);
        }

        // Xử lý rồng xương
        if (player.itemTime != null && player.itemTime.isUseRX) {
            newHpMax += (newHpMax * 10L / 100L);
        }

        // Xử lý set nhật ấn
        if (this.isNhatAn) {
            newHpMax += (newHpMax * 15L / 100L);
        }

        // Xử lý ngọc rồng đen 2 sao
        if (this.player.rewardBlackBall.timeOutOfDateReward[1] > System.currentTimeMillis()) {
            newHpMax += (newHpMax * RewardBlackBall.R2S_1 / 100L);
        }
        ...
        if (this.player.effectSkill.isMonkey) {
            if (!this.player.isPet || (this.player.isPet && ((Pet) this.player).status != Pet.FUSION)) {
                ...
                newHpMax += (newHpMax * percent / 100L);
            }
        }
        ...
        if (this.player.isPet && ((Pet) this.player).typePet == 4 && (((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA || ((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA2)) {
            newHpMax += (newHpMax * 40 / 100L);
        }

        // Xử lý phù
        if (this.player.zone != null && MapService.gI().isMapBlackBallWar(this.player.zone.map.mapId)) {
            newHpMax *= this.player.effectSkin.xHPKI;
        }
        ...
        if (this.player.fusion.typeFusion != ConstPlayer.NON_FUSION) {
            newHpMax += this.player.pet.nPoint.hpMax;
        }
        ...
        if (!this.player.isPet || (this.player.isPet && ((Pet) this.player).status != Pet.FUSION)) {
            if (this.player.effectSkill.tiLeHPHuytSao != 0) {
                newHpMax += (newHpMax * this.player.effectSkill.tiLeHPHuytSao / 100L);
            }
        }
        ...
        if (newHpMax > 2_000_000_000) {
            newHpMax = 2_000_000_000;
        }

        this.hpMax = (int) newHpMax;
    }
    ...
    private void setMpMax() {
        ...
        long newMpMax = this.mpg + this.mpAdd;

        // Áp dụng các yếu tố ảnh hưởng đến mpMax
        for (Integer tl : this.tlMp) {
            newMpMax += (newMpMax * tl / 100L);
        }

        if (this.isNguyetAn) {
            newMpMax += (newMpMax * 15L / 100L);
        }

        // Xử lý ngọc rồng đen 6 sao
        if (this.player.rewardBlackBall.timeOutOfDateReward[5] > System.currentTimeMillis()) {
            newMpMax += (newMpMax * RewardBlackBall.R6S_1 / 100L);
        }

        // Xử lý set worldcup
        if (this.player.setClothes.worldcup == 2) {
            newMpMax += (newMpMax * 10 / 100L);
        }
        ...
        if (this.player.isPet && ((Pet) this.player).typePet == 3
            ...
            newMpMax += (newMpMax * 20 / 100L);
        }

        // Xử lý pet mabư
        if (this.player.isPet && ((Pet) this.player).typePet == 1
                && (((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA || ((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA2)) {
            newMpMax += (newMpMax * 5 / 100L);
        }

        // Xử lý pet br
        if (this.player.isPet && ((Pet) this.player).typePet == 2
                && (((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA || ((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA2)) {
            newMpMax += (newMpMax * 30 / 100L);// MP berus
        }

        // Xử lý pet black
        if (this.player.isPet && ((Pet) this.player).typePet == 4
                && (((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA || ((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA2)) {
            newMpMax += (newMpMax * 40 / 100L);// MP black
        }

        // Xử lý phù
        if (this.player.zone != null && MapService.gI().isMapBlackBallWar(this.player.zone.map.mapId)) {
            newMpMax *= this.player.effectSkin.xHPKI;
        }
        ...
        // Xử lý rồng xương
        if (player.itemTime != null && player.itemTime.isUseRX) {
            newMpMax += (newMpMax * 10L / 100L);
        }

        // Xử lý hợp thể
        if (this.player.fusion.typeFusion != 0) {
            newMpMax += this.player.pet.nPoint.mpMax;
        }
        ...
        if (newMpMax > 2_000_000_000) {
            newMpMax = 2_000_000_000;
        }

        this.mpMax = (int) newMpMax;
    }
    ...
    private void setDame() {
        long newDame = this.dameg + this.dameAdd;

        for (Integer tl : this.tlDame) {
            newDame += (newDame * tl / 100L);
        }

        // Xử lý pet pic
        if (this.player.isPet && ((Pet) this.player).typePet == 3 && (((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA || ((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA2)) {
            newDame += (newDame * 20 / 100L);
        }

        // Xử lý pet mabư
        if (this.player.isPet && ((Pet) this.player).typePet == 1 && (((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA || ((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA2)) {
            newDame += (newDame * 5 / 100L);
        }

        // Xử lý pet br
        if (this.player.isPet && ((Pet) this.player).typePet == 2 && (((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA || ((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA2)) {
            newDame += (newDame * 30 / 100L);
        }

        // Xử lý pet black
        if (this.player.isPet && ((Pet) this.player).typePet == 4 && (((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA || ((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA2)) {
            newDame += (newDame * 40 / 100L);
        }

        // Xử lý set tinh ấn
        if (this.isTinhAn) {
            newDame += (newDame * 15L / 100L);
        }
        ...
        if (this.player.rewardBlackBall.timeOutOfDateReward[0] > System.currentTimeMillis()) {
            newDame += (newDame * RewardBlackBall.R1S_2 / 100L);
        }
        ...
        // Xử lý rồng xương
        if (player.itemTime != null && player.itemTime.isUseRX) {
            newDame += (newDame * 10L / 100L);
        }

        // Xử lý phù
        if (this.player.zone != null && MapService.gI().isMapBlackBallWar(this.player.zone.map.mapId)) {
            newDame *= this.player.effectSkin.xDame;
        }

        // Xử lý hợp thể
        if (this.player.fusion.typeFusion != 0) {
            newDame += this.player.pet.nPoint.dame;
        }
        ...
        if (this.player.effectSkill.isMonkey) {
            if (!this.player.isPet || (this.player.isPet && ((Pet) this.player).status != Pet.FUSION)) {
                ...
                newDame += (newDame * percent / 100L);
            }
        }

        //Sức đánh đẹp
        newDame += (newDame * tlSexyDame / 100L);

        // Xử lý giảm dame
        newDame -= (newDame * tlSubSD / 100L);
        ...
        if (newDame > 2_000_000_000) {
            newDame = 2_000_000_000;
        }

        this.dame = (int) newDame;
    }
...
//            }
    ...
    public long getPowerLimit() {
        return switch (limitPower) {
            case 0 -> 17999999999L; // Chưa mở giới hạn
            case 1 -> 19999999999L; // Đã mở giới hạn lần 1
            case 2 -> 24999999999L; // Đã mở giới hạn lần 2
            case 3 -> 29999999999L; // Đã mở giới hạn lần 3
            case 4 -> 39999999999L; // Đã mở giới hạn lần 4
            case 5 -> 50010000000L; // Đã mở giới hạn lần 5
            case 6 -> 60010000000L; // Đã mở giới hạn lần 6
            case 7 -> 70010000000L; // Đã mở giới hạn lần 7
            case 8 -> 80010000000L; // Đã mở giới hạn lần 8
            case 9 -> 90010000000L; // Đã mở giới hạn lần 9
            case 10 -> 190010000000L; // Đã mở giới hạn lần 10
            default -> 0;
        }; 
    }
    ...
    public int getHpMpLimit() {
        return switch (limitPower) {
            case 0 -> 220000; // Chưa mở giới hạn
            case 1 -> 240000; // Đã mở giới hạn lần 1
            case 2 -> 300000; // Đã mở giới hạn lần 2
            case 3 -> 350000; // Đã mở giới hạn lần 3
            case 4 -> 400000; // Đã mở giới hạn lần 4
            case 5 -> 450000; // Đã mở giới hạn lần 5
            case 6 -> 500000; // Đã mở giới hạn lần 6
            case 7 -> 525000; // Đã mở giới hạn lần 7
            case 8 -> 550000; // Đã mở giới hạn lần 8
            case 9 -> 575000; // Đã mở giới hạn lần 9
            case 10 -> 600000; // Đã mở giới hạn lần 10
            default -> 0;
        }; 
    }
    ...
    public int getDameLimit() {
        return switch (limitPower) {
            case 0 -> 11000; // Chưa mở giới hạn
            case 1 -> 12000; // Đã mở giới hạn lần 1
            case 2 -> 15000; // Đã mở giới hạn lần 2
            case 3 -> 18000; // Đã mở giới hạn lần 3
            case 4 -> 20000; // Đã mở giới hạn lần 4
            case 5 -> 22000; // Đã mở giới hạn lần 5
            case 6 -> 24000; // Đã mở giới hạn lần 6
            case 7 -> 24500; // Đã mở giới hạn lần 7
            case 8 -> 25000; // Đã mở giới hạn lần 8
            case 9 -> 25500; // Đã mở giới hạn lần 9
            case 10 -> 26000; // Đã mở giới hạn lần 10
            default -> 0;
        }; 
    }
    ...
    public short getDefLimit() {
        return switch (limitPower) {
            case 0 -> 550; // Chưa mở giới hạn
            case 1 -> 600; // Đã mở giới hạn lần 1
            case 2 -> 700; // Đã mở giới hạn lần 2
            case 3 -> 800; // Đã mở giới hạn lần 3
            case 4 -> 1000; // Đã mở giới hạn lần 4
            case 5 -> 1200; // Đã mở giới hạn lần 5
            case 6 -> 1400; // Đã mở giới hạn lần 6 
            case 7 -> 1500; // Đã mở giới hạn lần 7
            case 8 -> 1600; // Đã mở giới hạn lần 8
            case 9 -> 2000; // Đã mở giới hạn lần 9 
            case 10 -> 2500; // Đã mở giới hạn lần 10
            default -> 0;
        }; 
    }
