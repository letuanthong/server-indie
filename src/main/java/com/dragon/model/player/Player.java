...
import com.dragon.content.combine.Combine;
import com.dragon.consts.ConstAchievement;
import com.dragon.consts.ConstPlayer;
import com.dragon.consts.ConstTask;
import com.dragon.content.daihoi.The23rdMartialArtCongress;
import com.dragon.core.data.DataGame;
import com.dragon.content.dungeon.MajinBuu14H;
import com.dragon.content.event.shenron.Shenron_Event;
import com.dragon.core.interfaces.IPVP;
import com.dragon.content.intrinsic.IntrinsicPlayer;
import com.dragon.model.item.Item;
import com.dragon.model.item.ItemTime;
...
import com.dragon.model.mob.MobMe;
import com.dragon.core.network.Message;
import com.dragon.core.network.MySession;
import com.dragon.model.npc.MabuEgg;
import com.dragon.model.npc.MagicTree;
import com.dragon.model.npc.NonInteractiveNPC;
import com.dragon.model.player.badges.Badges;
import com.dragon.model.player.badges.BadgesData;
...
import com.dragon.model.skill.PlayerSkill;
import com.dragon.model.skill.Skill;
import com.dragon.content.task.BadgesTask;
import com.dragon.content.task.BadgesTaskService;
import com.dragon.content.task.TaskPlayer;
        ...
        if (!this.beforeDispose) {
            try {
                ...
                if ((this.zone != null && !MapService.gI().isHome(this.zone.map.mapId)) || (!this.isPl() && this.zone == null)) {
                    ...
                    if (Util.canDoWithTime(lastTimeChibi, 300000) && Util.isTrue(0, 2000)
    ...
    protected void setDie(Player plAtt) {
        if (this.isPl()) {
            ...
            if (this.inventory.gold >= vang && vang >= 1) {
                ...
                vang = vang * 95 / 100;
                if (vang < 10000) {
                    Service.gI().dropItemMap(this.zone, ItemMap.create(zone, 189, vang, this.location.x, this.location.y, this.id));
                } else if (vang < 20000) {
                    Service.gI().dropItemMap(this.zone, ItemMap.create(zone, 188, vang, this.location.x, this.location.y, this.id));
                } else {
                    Service.gI().dropItemMap(this.zone, ItemMap.create(zone, 190, vang, this.location.x, this.location.y, this.id));
                }
            }
        }
    ...
    public void spreadEffectToNearbyPlayers() {
        if (hasEffect(this, 7143)) {
            try {
                ...
                System.out.println("[DEBUG] Lỗi khi lan hiệu ứng 7143: " + e.getMessage());
            }
        } else {
