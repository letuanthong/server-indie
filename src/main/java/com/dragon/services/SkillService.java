...
import com.dragon.boss.broly.SuperBroly;
import com.dragon.boss.superrank.Rival;
import com.dragon.boss.yardat.Yardart;
...
import com.dragon.model.player.NewSkill;
...
public class SkillService {
            ...
            if (canHsPlayer(player, plTarget)) {
                ...
                for (Player pl : players) {
                    try {
                        ...
                        player.nPoint
                                .setHP(player.nPoint.getHP() + ((long) player.nPoint.hpMax * percentTriThuong / 100));
    ...
    private void playerAttackPlayer(Player plAtt, Player plInjure, boolean miss) {
        ...
        if (plAtt.isPl() && dameHit >= 250_000_000) {
            ServerNotify.gI().notify(plAtt.name + ": đã đánh 1 chiêu " + skillSelect.template.name
                    + " với sát thương là " + Util.formatNumber(dameHit));
        }
    ...
    private void playerAttackMob(Player plAtt, Mob mob, boolean miss, boolean dieWhenHpFull) {
        ...
        // Xử lý hiệu ứng tăng sát thương (nếu có)
