...
import com.dragon.consts.ConstItem;
...
public class UseItem {
                    ...
                    default -> {
                        switch (item.template.id) {
                            ...
                            case 1559: // capule kich hoat
                                ...
                                UseItem.gI().Gwen_PhaoHoa(pl, item);
                                break;
                            case 1750:
                                InventoryService.gI().itemBagToBody(pl, indexBag);
                            ...
                            case 1508:
                            case 1509:
                                NpcService.gI().createMenuConMeo(pl, ConstNpc.EVENT_3_1, -1,
    ...
    private void Gwen_PhaoHoa(Player pl, Item item) {
        int[][] gold = { { 5000, 20000 } };
        ...
        Service.gI().Gwen_LogicEffect(pl, 1011, 1, -1, 1, 1, 15000);
        Service.gI().Gwen_LogicEffect(pl, 63, 1, -1, 1, 1, 5000);
        Service.gI().Gwen_LogicEffect(pl, 64, 1, -1, 1, 1, 5000); // eff này live
        Service.gI().Gwen_LogicEffect(pl, 65, 1, -1, 1, 1, 5000);
            ...
            for (Player bossPlayer : bosses) {
                if (bossPlayer.id == BossID.SOI_HEC_QUYN) {
                    ...
                    if (soihecQuyn != null) {
                        ...
                        itemMap = ItemMap.create(pl.zone, 460, 1, x, y, pl.id);
        ...
        if (InventoryService.gI().getCountEmptyBag(pl) > 0) {
            ...
            if (itemwhis == 746) { // Xe trượt tuyết
                if (Util.isTrue(1, 100)) { // 1% ra HSD vĩnh viễn
                    ...
                    || itemwhis == 1018 || itemwhis == 1019 || itemwhis == 1020) { // Cải trang Noel hoặc Broly
            ...
            } else if (itemwhis >= 1087 && itemwhis <= 1191) { // Diệt Quỷ
    ...
    public void CapsuleKichHoat(Player player) {
        ...
        if (InventoryService.gI().getCountEmptyBag(player) >= emtyBag) {
            ...
            // short itemId = ConstItem.TrangBiKichHoat[Util.nextInt(0,
            // 4)][player.gender][Util.nextInt(0, ConstItem.TrangBiKichHoat[0][0].length -
            // 1)];
            // Item ID_SetKichHoat = ItemService.gI().createNewItem(itemId);
